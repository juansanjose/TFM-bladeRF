...
int trx_brf_set_freq(openair0_device* device, openair0_config_t *openair0_cfg1)
{
    ...
    if ((status=bladerf_set_frequency(brf->dev, BLADERF_MODULE_TX, (unsigned int)  4300000000)) != 0) {
        fprintf(stderr,"Failed to set TX frequency: %s\n",bladerf_strerror(status));
        brf_error(status);
    } else
        printf("[BRF] set TX Frequency to %u\n", (unsigned int)  4300000000);

    if ((status=bladerf_set_frequency(brf->dev, BLADERF_MODULE_RX, (unsigned int)   3500000000)) != 0) {
        fprintf(stderr,"Failed to set RX frequency: %s\n",bladerf_strerror(status));
        brf_error(status);
    } else
        printf("[BRF] set RX frequency to %u\n",(unsigned int)  3500000000);
...
void calibrate_rf(openair0_device *device)
{
    ...
    bladerf_set_frequency(brf->dev,BLADERF_MODULE_TX, (unsigned int)  430000000);
    ...
    bladerf_set_frequency(brf->dev,BLADERF_MODULE_TX, (unsigned int)  430000000 + (unsigned int)  7680000/4);
    ...
    bladerf_set_gain(brf->dev, BLADERF_MODULE_TX, (unsigned int) 60);
                ...
                openair0_config_t *openair0_cfg)
{
    ...
    case 30720000:
        ...
        break;
    case 15360000:
        openair0_cfg->samples_per_packet    = 2048;//2048
        openair0_cfg->tx_sample_advance     = 0;
        break;
    case 7680000:
        openair0_cfg->samples_per_packet    = 1024;//1024
        openair0_cfg->tx_sample_advance     = 0;
        break;
    case 1920000:
    ...
    brf->rx_timeout_ms = 0;
    brf->tx_timeout_ms = 0;
    brf->sample_rate=(unsigned int)  7680000;
    ...
    if ((status=bladerf_set_frequency(brf->dev, BLADERF_MODULE_RX, (unsigned int)  350000000)) != 0) {
        fprintf(stderr,"Failed to set RX frequency: %s\n",bladerf_strerror(status));
        brf_error(status);
    } else
        printf("[BRF] set RX frequency to %u\n",(unsigned int) 350000000);



    unsigned int actual_value=0;
    
    if ((status=bladerf_set_sample_rate(brf->dev, BLADERF_MODULE_RX, (unsigned int)   7680000, &actual_value)) != 0) {
        fprintf(stderr,"Failed to set RX sample rate: %s\n", bladerf_strerror(status));
        brf_error(status);
    } else
        printf("[BRF] set RX sample rate to %u, %u\n", (unsigned int)   7680000, actual_value);


    if ((status=bladerf_set_bandwidth(brf->dev, BLADERF_MODULE_RX, (unsigned int)   25000000, &actual_value)) != 0) {
        fprintf(stderr,"Failed to set RX bandwidth: %s\n", bladerf_strerror(status));
        brf_error(status);
    } else
        printf("[BRF] set RX bandwidth to %u, %u\n",(unsigned int)  25000000, actual_value);
    ...
    if ((status=bladerf_set_frequency(brf->dev, BLADERF_MODULE_TX, (unsigned int)  430000000)) != 0) {
        fprintf(stderr,"Failed to set TX frequency: %s\n",bladerf_strerror(status));
        brf_error(status);
    } else
        printf("[BRF] set TX Frequency to %u\n", (unsigned int)  430000000);

    if ((status=bladerf_set_sample_rate(brf->dev, BLADERF_MODULE_TX, (unsigned int)   7680000, NULL)) != 0) {
        fprintf(stderr,"Failed to set TX sample rate: %s\n", bladerf_strerror(status));
        brf_error(status);
    } else
        printf("[BRF] set TX sampling rate to %u \n", (unsigned int) 7680000);

    if ((status=bladerf_set_bandwidth(brf->dev, BLADERF_MODULE_TX,(unsigned int)25000000, NULL)) != 0) {
        fprintf(stderr, "Failed to set TX bandwidth: %s\n", bladerf_strerror(status));
        brf_error(status);
    } else
        printf("[BRF] set TX bandwidth to %u \n", (unsigned int) 25000000);

    if ((status=bladerf_set_gain(brf->dev, BLADERF_MODULE_TX, (int) 60)) != 0) {
    ...
    // if ((status=bladerf_sync_config(brf->dev, BLADERF_MODULE_TX,BLADERF_FORMAT_SC16_Q11_META,128,1024,16,0)) != 0 ) {
    //     fprintf(stderr,"Failed to configure TX sync interface: %s\n", bladerf_strerror(status));
    //     brf_error(status);
